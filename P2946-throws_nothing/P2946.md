...
% <!-- $TimeStamp$ -->2023-07-19 13:04 EDT<!-- $ -->
...
alone, such as embedded developers who want smaller code and those who need to
...
memory-constrained environments.  WG21 members often assert that embedded developers
...
Since even an embedded processor may have several megabytes of RAM,
completely turning off exceptions is not always necessary. Given
an appropriate C++ implementation, judicious use of the `[[throws_nothing]]`
attribute can reduce the size of an
executable while following the best design practices, including
the Lakos Rule.
...
i.e., by providing inputs that at the boundaries of the precondition, including
deliberately violating it. When every precondition violation causes program
termination, it is not possible to write a portable and efficient test driver,
...
change the program logic within the caller, the test engineer can have
...
feature, `[[throws_nothing]]`, that differs only in that (1) it cannot be
