...
% <!-- $TimeStamp$ -->2023-07-17 18:02 EDT<!-- $ -->
...
annotation than `noexcept` that avoids the issues that the Lakos Rule was
created to address.  Thus, `[[throws_nothing]]` eliminates the temptation to
abuse `noexcept` by providing a tool better suited than `noexcept` for
improving code generation and documenting programmer intent.

Brief Summary
=============

This minimal summary of the `[[throws_nothing]]` feature is sufficient for
...
It is implementation defined whether the program will terminate if an exception
attempts to escape from `f`, above.
 ...
 * If a function has preconditions (a *narrow contract*) or it might
   throw when called correctly (*in contract*), it may not be declared
   `noexcept`.
...
a facility (see [P2831R0](http://wg21.link/P2831R0)).
...
it is not always necessary to completely turn off exceptions. Assuming
an appropriate C++ implementation, judicious use of the `[[throws_nothing]]`
attribute can reduce the size of an
...
test engineer would thus choose a C++ implementation that ignores
...
If a `[[throws_nothing]]` function attempts to exit via an exception, it is
...
If the function is also annotated with
`noexcept` or `noexcept(true)`, however, `std::terminate` is always called,
...
Changes are relative to the May 2023 Working Paper,
[N4950](http://wg21.link/N4950)

**Note**: This wording is known to be incomplete.  Where known open issues are
...
declared with the `throws_nothing` attribute, it is implementation-defined
whether the function `std::terminate` is invoked ([except.terminate]).

**Open issue**: Does anything need to be said about such a function called
indirectly via a function pointer?

Insert the following new subsection at the end of the **[dcl.attr]**
section:
...
The effects of the `throws_nothing` attribute is described in [except.spec].
...
wording for contracts; see [P2877R0](http:/wg21.link/P2877R0).
