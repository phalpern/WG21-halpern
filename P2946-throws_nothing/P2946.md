...
<!---
    MG: be consistent regarding Lakos Rule hyphenation.
-->
...
```
<!---
    MG: In the above code, the variable `success` is not used, 
        suggest removing.
-->
...
<!---
    MG: Is there any point talking about the original spelling? I'd be tempted 
        to either leave it out completely, or relegate it into a footnote or 
...
<!---
    MG: is this really what you want? It provides a way to programatically query,
        using __has_cpp_attribute(throws_nothing), whether you are in a build
        mode that would call std::terminate were an exception to be thrown in
