...
% <!-- $TimeStamp$ -->2023-07-15 13:05 EDT<!-- $ -->
...
If a `[[throws_nothing]]` function attempts to exit with an exception, the
program will either terminate (enforce semantics) or the exception will be
propagated as usual (ignore semantics), depending on the implementation. A
recommended best practice is to make both semantics available to the user, with
enforce semantics being the default.
...
does not affect *essential behavior*[^essential]. The `[[throws_nothing]]`
attribute can, therefore, be added to or removed from a function declaration
...
[^essential]: Essential behavior comprises the promised behavior of a
function when called in contract.  The return value, guaranteed side-effects,
and complexity guaranties are part of essential behavior.  The layout of
objects, number of instructions executed, and logging are rarely part of
its essential behavior. The effects of calling the
function out of contract are *never* part of essential behavior.
...
function exits via a thrown exception, it is *implementation
defined* whether the exception propagates or `std::terminate` is called.

Critical to the `[[throws_nothing]]` feature is that its use cannot
...
always terminating or letting the behavior be
...
provide better choices for various use cases.
