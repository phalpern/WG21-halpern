...
<!-- lah: title? authors? etc. --> 

Abstract
========

The `noexcept` function specifier and companion `noexcept` operator were
invented to allow containers such as `vector` to provide the <!-- lah: replace "the" with "a"? unless this is a proper name or there is only one such thing. If you change it here, then search for others and change as needed. --> 
strong
...
code generation and, sometimes, as a form of documentation.  <!-- lah: if we add "regularly" after "utilized", then we can delete the commas around sometimes AND have a more parallel sentence structure. --> 
The problems with
these off-label uses have been known for a long time; the *Lakos
Rule*, which predated the release of C++11, was intended to prevent such use
(or misuse) within the C++ Standard Library.  In this paper, I propose an <!-- lah: Here "this paper" is part of a prep phrase and you use first person "I". In other places, "this paper" is the subject, not "I", which can get confusing because sometimes I wonder if you mean "this paper" presents x, not y, but YOU plan to present y in a followup paper. --> 
annotation for nonthrowing functions to be used instead of `noexcept` in
circumstances where the latter is undesirable --- <!-- lah: em dash, not en dash --> 
...
facility. <!-- lah: why is throws-nothing plain text here, not `[[throws_nothing]]`? -->

Brief Summary
=============

I propose an annotation in the form of an attribute, `[[throws_nothing]]`, <!-- lah: there's some kind of issue here with the md conversion because the highlighting of code terms means there's now a space between the closing bracket and the comma that shouldn't be there. I guess it's extra padding for the highlighting, but it's not good. --> 
indicating that a function does not throw when called properly, i.e., *in
contract*: <!-- lah: if it's important enough to be in ital, then it's important enough to be nonparenthesized. --> 
...
does not affect *essential behavior* and may <!-- lah: "may" indicates permission; "might" indicates questionable likelihood. Which is right here? --> 
be ignored by some platforms,
...
purpose: to enable the safe use of move constructors in `vector`-like <!-- lah: here's another instance where the highlighting makes it look like there's a space between vector and the hyphen. --> 
...
[N2855](http://wg21.link/n2855), the problem was first
described and `noexcept` was first proposed. <!-- lah: putting the prep phrase at the beginning makes it very clear that BOTH things happened for the first time in N2855. --> The proposed `noexcept` wording was later
...
implementation uses `if constexpr` <!-- lah: Note John might want to change this to `if` `constexpr` (to get a text space rather than a code space between words, and if he does make that change, it could add a white space between two code highlighted blocks and look even worse. --> 
instead of the `std::move_if_noexcept`
function to make the two different code paths easier to distinguish.

<!-- lah: Will you be submitting a PDF, and if so, would this code fit? --> 
```C++
...
to using the copy constructor, giving up the performance gain to 
ensure that it can leave the original object in its initial state. <!-- lah: The "however" is in a weird place. Do you mean "The move constructor, however, modifies the original object" or do you mean "The move constructor modifies the original object, but if the move constructor might throw, `vector`..." --> 
Because this <!-- lah: "this" what? example? situation? --> 
is a *generic context*, using `std::move` and
retaining the strong guarantee in the code above would be impossible if we did not have the `noexcept`
operator.

The Lakos Rule
--------------

Since the `noexcept` annotation was added late in the C++11 cycle and was thus brand new and not fully
 ...
 * If a function has no preconditions (a *wide contract*) and is guaranteed not to
...
for violating it in the Standard Library design.
 ...
 1. Under many (but by no means all) circumstances, calling a `noexcept`
    ...
    I've seen no compelling evidence that `noexcept` produces
...
safer than undefined behavior but might still be unsafe in a critical system that must not
...
itself does not mandate its use. In fact, for discretionary use by implementations, `[[throws_nothing]]` is much better than the `noexcept` specifier
because `[[throws_nothing]]` cannot inadvertently change the meaning of a program and is
...
implied. Initially, it would be expected (and encouraged) <!-- lah: This kind of construction is passive language to the 10th degree. Expected by whom and of whom? "Initially, developers would be expected and encouraged to provide in every implementation at least two..." That's still passive but less so. --> 
that every
implementation
...
2. **Ignore/rethrow**: Allow the exception to escape, as though the annotation
   were not present. <!-- lah: Replace the slash with "and", "or", or "and/or" as appropriate. --> 

Other options, some of which are described in the
[Future Integration with Contracts](#future-integration-with-contracts)
section below, are available.

Mixing build modes can be dangerous; a caller that is built in *terminate*
mode might exhibit undefined behavior when calling a `[[throws_nothing]]` function in a
...
link. Alternatively, the implementation could always generate, in addition to any other version, a "safe" version
that terminates and again use name mangling to
ensure that the correct version is seen by the caller. The Standard, not
recognizing build modes, would have nothing to say on that subject. <!-- lah: "have nothing to say on that subject" is soemwhat clichéd. How about "would be moot"?  -->
...
Though putting the `[[throws_nothing]]` attribute in the
same location as `noexcept` would seem logical, for an attribute to appertain to a
function, the attribute must occur either before the function declaration or immediately
...
to be the same number of characters as `[[throws_nothing]]`. <!-- lah: does the matching number of characters matter? --> 
The name was
...
clause is. Such functionality seems counterintuitive; there is deliberately no <!-- lah: Who deliberately created it this way? Example: "I deliberately omitted any method for querying..." -->  
way to query the `[[throws_nothing]]` attribute, thus the common idiom of
...
`[[throws_nothing(` *constant-bool-expression* `)]]`. <!-- lah: Look at this term in the displayed md version and make sure it looks as you intended, e.g., the spaces around constant-bool-expression, the interruption in code font, etc. --> 
...
exception through an otherwise-nonthrowing interface. One proposed way to achieve 
...
overlap in a number of ways, and suggestions have emerged whereby these features can be harmonized,
resulting in a more powerful and consistent Standard.

* `[[throws_nothing]]` can be considered a special type of contract check ---
  similar to the Contracts proposal's `pre` and `post` --- but evaluated at a <!-- lah: pre and post what? This seems incomplete. --> 
  distinct time, i.e., when an exception tries to escape. The proposed
  Contracts facility gives the programmer control, via a
  user-replaceable violation handler, over how detected logic errors are managed, 
  e.g., logging and terminating, logging and somehow continuing, and so on.
  Affording the user the same level of control
  over how an escaping exception is handled in a `[[throws_nothing]]`
  context would be a powerful feature. Thus, in addition to propagate mode and terminate mode, an integrated facility
...
* Integration between `[[throws_nothing]]` and contract annotations would be more
  seamless if their syntaxes were harmonized. If the Contracts facility ends up using a
  nonattribute syntax, then *throws-nothing* <!-- lah: Why ital text? Why not `[[throws_nothing]]`? --> 
  might use a similar syntax
  (assuming the two features first ship in the same Standard). As described
  earlier, `[[throws_nothing]]` cannot, in today's C++, appear at the end of a function declaration
  (my preferred position) <!-- lah: Note first person is used again here, not "this paper's preferred position" --> 
  and still appertain to the function.
...
* In a build mode in which contract-checks are enabled and a throwing
  ...
  contract-violation exceptions to be allowed to escape a `[[throws_nothing]]`, <!-- lah: A `[[throws_nothing]]` what? Function? Either add a word or delete the "a". --> 
...
code. This line is also the least safe because, just as in the case of other contract
...
No changes would be needed immediately in the Standard Library if this facility <!-- lah: I would change "this" to `[[throws_nothing]]` unless there is a reason not to. The next senence makes me think perhaps there is such a reason. --> 
...
An immediate change might be necessary if 
any narrow-contract functions targeted for C++26 are currently annotated
with `noexcept`; perhaps those annotations should be changed to 
`[[throws_nothing]]` or perhaps developers should be allowed to decide whether to use `[[throws_nothing]]` in those implementations.  <!-- lah: This sentence had too many clauses that didn't work well; make sure I understood your intention accurately. --> 
Violations of the Lakos
Rule already in C++23 could be handled on a case-by-case basis (via
DRs). Minimizing such violations would result in greater stability across
...
favorable response in EWGI, we will implement the proposed facility before presenting 
