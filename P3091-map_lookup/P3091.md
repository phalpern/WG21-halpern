...
document: D3091R4
date: <!-- $TimeStamp$ -->2025-06-20 00:41 EEST<!-- $ -->
audience: LEWG
author:
  - name: Pablo Halpern
...
The most convenient way to look up an element of a `map`-like container is
...
and monadic operations of `optional` to simplify lookups for `map`,
`unordered_map`, and `flat_map`. Note that this proposal is built on
`optional<T&>`, which is
described in [@P2988R12] and is expected to be voted into the C++ Working Draft
in Sofia. It's usefulness is also enhanced by the `maybe` facilities described
in [@P1255].
...
- Record vote in LEWG to keep `.get` instead of changing to `.lookup`.
