...
document: D3091R3
date: <!-- $TimeStamp$ -->2024-09-17 16:18 MDT<!-- $ -->
audience: LEWGI
author:
  - name: Pablo Halpern
...
`unordered_map`. Note that this proposal is built on `optional<T&>`, which is
described in [@P2988R6] and is not yet in the C++ Working Draft.
...
- Removed wording that duplicates parts of [@P2988R6].
- Removed enhancements to `optional<T>::value_or` that will be handled in a
  separate paper, [@P1255R13], by Steve Downey.
...
instantiated with reference types, as proposed in [@P2988R6].
...
non-member function that is expected to be in the next revision of [@P1255R13]
by Steve Downey. Since this change does not directly relate to the subject of
this proposal, there is no reason to combine it with this paper.  You can see
the proposed extensions in a previous revision of this paper, [@P3091R1].
...
```cpp
T& x = std::reference_or(m.get(k), v);  // per @[@P1255R13]
```
...
```cpp
map<K, vector<U>> m{ ... };
span<U> x = value_or<span<U>>(m.get(k));
```
...
const array<U, N> preset{ ... };
span<const U> x =
  value_or<span<const U>>(m.get(k), preset);
```
...
```cpp
unordered_map<K, U*> m{ ... };
if (U* p = value_or(m.get(k), nullptr); p) {
  // use p ...
}
...
- R0 `map` and `unordered_map` specification changes would be independent of
  `optional<T&>`, thus decoupling this paper from [@P2988R6] and its successor
  [@P2988R5].
...
- By returning `optional`, the current design can leverage all of the
  ...
  without modifying `map`.  In particular, all of the features in [@P1255R13]
...
Working Draft, [@N4971], or to the paper proposing `optional<T&>`, [@P2988R6].
...
In the `optional<T&>` wording *in [@P2988R6]*, change `value_or` in
...
In the `optional<T&>` wording *in [@P2988R6]*, change the
