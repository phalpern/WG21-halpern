...
document: D3091R2
date: <!-- $TimeStamp$ -->2024-05-21 16:48 EDT<!-- $ -->
audience: LEWGI
author:
  - name: Pablo Halpern
...
The return value of `value_or` is `T&` (as proposed in [@P2988R4]). That means
...
in [@P2988R4] is difficult to use. If `T` is non-const, a valid-looking and
...
reference, [@P2988R4] says that the return value of `value_or` is also a
...
from `A0` to `U` does not
>> construct

`reference_constructs_from_temporary_v<U, A0>`
>> is `false`.
