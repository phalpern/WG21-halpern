...
document: D3091R1
date: <!-- $TimeStamp$ -->2024-03-08 16:56 EST<!-- $ -->
audience: LEWGI
author:
  - name: Pablo Halpern
...
```cpp
template <class U = remove_reference_t<T>, class... Args>
...
>> *Mandates*: `is_constructible<U, decltype(*val)> &&`
>> `is_constructible<U, Args...>` is `true`.

>> *Effects*: Equivalent to:

>>> `return has_value() ? U(*val) : U(std::forward<Args>(args)...);`
:::
...
>> *Mandates*:
>> [`is_constructible_v<add_lvalue_reference_t<T>, decltype(v)>` is `true`]{.rm}
>> [`common_reference_t<add_lvalue_reference_t<T>, decltype(v)>` exists
>> and is an lvalue reference type]{.add}.

>> *Effects*: [Let *R* be `common_reference_t<add_lvalue_reference_t<T>, decltype(v)>`.]{.add} Equivalent to:
...
Thanks to Tomasz Kamiński for pushing me on the `allocator<T&>` approach.
