...
<!-- $TimeStamp$ -->2022-02-23 17:51 EST<!-- $ --> \
...
`aligned_object_storage` was included for this purpose. In response to a
reviewer inquiry, it was decided that this template should be instantiatable
with complete object types only, and that cv-qualifications *would* be allowed
on the parameter type. The rationale for the latter is to facilitate generic
programming with possibly cv-qualified types. Moreover, there was no technical
reason to forbid cv-qualification, as the `data()` method returns the
underlying representation (raw storage) as an (unqualified) pointer to
`void`. The prototype implementation includes tests showing that cv-qualified
types create no implementation or usage difficulties.
