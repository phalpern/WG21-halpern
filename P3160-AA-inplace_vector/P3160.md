...
document: D3160R2
date: <!-- $TimeStamp$ -->2024-10-13 12:21 EDT<!-- $ -->
audience: LEWG
author:
  - name: Pablo Halpern
...
Add an `Allocator` parameter to the definition of `inplace_vector`. Note that, **unlike other allocator-aware containers**, the `pointer` and `size_type` types are not taken from the allocator because the elements of the `inplace_vector` are not stored in allocated memory:
...
Update the Constructors section as follows. **Drafting Note**: the addition of the move constructor corrects a pre-existing issue with the `inplace_vector` move constructor -- that, unlike for other containers, it does not have constant complexity**:
...
> *Preconditions*: `T` is *Cpp17MoveInsertable* into `inplace_vector`.

> *Effects*: Move construct elements in `*this` from the corresponding elements in `v` using the allocator obtained from `v.allocator()`.

> *Complexity*: Linear in `v.size()`

| `constexpr inplace_vector(inplace_vector&& v, const type_identity_t<Allocator>&)`
|     `noexcept(N == 0 || @*is-nothrow-ua-constructible-v*@<T, Allocator, T&&>);`

> *Preconditions*: `T` is *Cpp17MoveInsertable* into `inplace_vector`.

> *Effects*: Construct elements in `*this` from rvalue references to the corresponding elements in `v` using the specified allocator.

> *Complexity*: Linear in `v.size()`

| `constexpr inplace_vector& operator=(const inplace_vector& other)`
|     `noexcept(N == 0 || (is_nothrow_copy_assignable_v<T> &&`
|                                          `@*is-nothrow-ua-constructible-v*@<T, Allocator, const T&>));`
...
Add the definition of `swap` to the end of [inplace.vector.modifiers]{.sref}.  **Drafting Note**: this definition of swap, in addition to addressing the allocator, also fixes a pre-existing issue with `inplace_vector::swap` -- that, unlike for other containers, it does not have constant complexity**:
...
> *Complexity*: Linear in `max(size(), x.size())`.
:::
