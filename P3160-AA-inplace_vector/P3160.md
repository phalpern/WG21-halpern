...
document: D3160R2
date: <!-- $TimeStamp$ -->2024-10-09 21:19 EDT<!-- $ -->
audience: LEWG
author:
  - name: Pablo Halpern
...
Arthur wants two more that I don't really understand:
...
> [empty. If `N` is zero and either `is_trivial_t<A>` is `true` or `A` is a
> specialization of `std::allocator`, then `inplace_vector<T, N@[, A]{.add}@>`
> is]{.add} both trivial and empty. Otherwise:

>  - If `is_trivially_copy_constructible_v<T>` [`&& is_trivially_copy_constructible_v<A>`]{.add} is `true`, then `IV` has a trivial copy constructor.
...
Add the allocator to the definition of `inplace_vector`:
...
>| `constexpr inplace_vector() noexcept;`
>| [`constexpr inplace_vector(const Allocator&) noexcept;`]{.add}
...
>|     `constexpr inplace_vector(InputIterator first, InputIterator last`[`, const Allocator& = Allocator()`]{.add}`); //` *freestanding-deleted*
>| `template<@*container-compatible-range*@ <T> R>`
>|     `constexpr inplace_vector(from_range_t, R&& rg`[`, const Allocator& = Allocator()`]{.add}`); //` *freestanding-deleted*
>| `constexpr inplace_vector(const inplace_vector&);`
>| [`constexpr inplace_vector(const inplace_vector&, const Allocator&);`]{.add}
>| `constexpr inplace_vector(inplace_vector&&)`
>|   [`noexcept(N == 0 || is_nothrow_move_constructible_v<T>)`]{.rm}
>|   [`noexcept(N == 0 || noexcept(make_obj_using_allocator(declval<const Allocator&>(), declval<T>())))`]{.add}`;`
>| [`constexpr inplace_vector(inplace_vector&&, const Allocator&)`]{.add}
>|   [`noexcept(N == 0 || noexcept(make_obj_using_allocator(declval<const Allocator&>(), declval<T>())));`]{.add}
...
Also update the `noexcept` clause for the `swap` member function and `swap`
friend function.

>| `constexpr void swap(inplace_vector& x)`
>|     `noexcept(N == 0 || (is_nothrow_swappable_v<T> &&`
>|                                          [`is_nothrow_move_constructible_v<T>`]{.rm}
>|                                          [`noexcept(make_obj_using_allocator(declval<const Allocator&>(), declval<T>()))`]{.add}`));`
...
|      `noexcept(N == 0 || noexcept(make_obj_using_allocator(declval<const Allocator&>(), declval<T>())));`
...
| `constexpr void swap(inplace_vector& x)`
|     `noexcept(N == 0 || (is_nothrow_swappable_v<T> &&`
|                                          `noexcept(make_obj_using_allocator(declval<const Allocator&>(), declval<T>()))));`
