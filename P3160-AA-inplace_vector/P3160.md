...
document: D3160R2
date: <!-- $TimeStamp$ -->2024-10-11 12:57 EDT<!-- $ -->
audience: LEWG
author:
  - name: Pablo Halpern
...
+-----------------------------------------------------------------------------+
| **Design Note**: The wording below assumes the *uses-allocator* design. Where possible, expected changes for the `construct/destroy` design are called out in other **Design Note** passages similar to this one. |
+-----------------------------------------------------------------------------+
...
+-----------------------------------------------------------------------------+
| **Design Note**: the second addition above is needed for the *uses-allocator* design of `inplace_vector`; it should be omitted if the `construct/destroy` design is chosen. |
+-----------------------------------------------------------------------------+
...
+-----------------------------------------------------------------------------+
| **Design Note**: The changes to *Note 2* apply to the *uses-allocator* design only; not to the `construct/destroy` design. |
+-----------------------------------------------------------------------------+
...
> treated as being trivially default constructible, trivially copyable,
> trivially movable, and trivially
> destructible. [*Note*: for implementations having non-trivial special member
> functions in `std::allocator`, `inplace_vector` might not store a
> `std::allocator` object, but could use a new, value-initialized object
> each time a `std::allocator` object is required --- *end note*]

> Let `IV` denote a specialization of `inplace_vector<T, N@[, A]{.add}@>`. If
> `N` is zero, then `IV` is [empty. If `N` is zero and `is_trivial_v<A>` is
> `true`, then `inplace_vector<T, N@[, A]{.add}@>` is]{.add} both trivial and
> empty. Otherwise:
...
+-----------------------------------------------------------------------------+
| **Design Note**: The *is-nothrow-ua-constructible-v* trait, above, is sufficient for the *uses-allocator* design.  The `construct/destroy` design would need a different trait (e.g., *is-nothrow-allocator-construct*) whose result would depend on the presence or absence of a `construct` member function as well as the `noexcept` specification for such a member.  Because `construct` does not have a `noexcept` specification in the existing standard library, such a trait would need to have specializations for the standard allocators, `scoped_allocator_adaptor` and `pmr::polymorphic_allocator`.  For allocators that don't supply `construct`, this new trait would be equivalent to `is_nothrow_constructible`. |
+-----------------------------------------------------------------------------+
...
+-----------------------------------------------------------------------------+
| **Design Note**: The behavior of `swap` described above works for the *uses-allocator* design, but not for the `construct/destroy` design. In the latter case, if the allocator propagates on swap, we need an additional mandate that it is always possible to construct (not allocate) an object using one allocator and destroy (not deallocate) it using a different allocator of the same type, as described in the section, [Behavior of move assignment and `swap`](#behavior-of-move-assignment-and-swap).  The *Effects* clause will be largely the same, but a complex *Mandates* clause would likely be needed. |
+-----------------------------------------------------------------------------+
