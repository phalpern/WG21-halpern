...
document: D3160R2
date: <!-- $TimeStamp$ -->2024-10-10 16:36 EDT<!-- $ -->
audience: LEWG
author:
  - name: Pablo Halpern
...
**Design Note**: The wording below assumes the *uses-allocator* design. Where
possible, expected changes for the `construct/destroy` design are called out in
other **Design Note** passages similar to this one.

Feature-test macros
-------------------

In [version.syn]{.sref}, add a feature test macro for freestanding `allocator`
and update the version date for `inplace_vector`. In both cases, `20XXXXL`
represents the year and month that this paper is adopted into the WP:
...
In response to Ben Craig, Arthur wants two more that I don't really understand:
`__cpp_lib_freestanding_inplace_vector` ("At least the freestanding subset of
`inplace_vector` is available (but, e.g., the *freestanding-deleted*
iterator-pair constructor might be `=deleted`)") and `__cpp_lib_allocator`
("`std::allocator` is fully implemented (including `allocate` and
`deallocate`)"). They seem to indicate something other than conformance to a
specific draft of the WP, so I'm not sure they're really feature-test macros.
...
**Design Note**: the second addition above is needed for the *uses-allocator*
design of `inplace_vector`; it should be omitted if the `construct/destroy`
design is chosen.
...
> An `inplace_vector` meets all of the requirements of a container (24.2.2.2),
...
> `N` is zero, then `IV` is
> [empty. If `N` is zero and either `is_trivial_v<A>` is `true` or `A` is a
> specialization of `std::allocator`, then `inplace_vector<T, N@[, A]{.add}@>`
> is]{.add} both trivial and empty. Otherwise:
...
**Design Note**: The *is-nothrow-ua-constructible-v* trait, above, is
sufficient for the *uses-allocator* design.  The `construct/destroy` design
would need a different trait (e.g., *is-nothrow-allocator-construct*) whose
result would depend on the presence or absence of a `construct` member function
as well as the `noexcept` specification for such a member.  Because `construct`
does not have a `noexcept` specification in the existing standard library, such
a trait would need to have specializations for the standard allocators,
`scoped_allocator_adaptor` and `pmr::polymorphic_allocator`.  For allocators
that don't supply `construct`, this new trait would be equivalent to
...
### Changes to Constructors ([inplace.vector.cons]{.sref})

Update the Constructors section as follows. **Note that the addition of the
move constructor corrects a pre-existing issue with the `inplace_vector` move
constructor -- that, unlike for other containers, it does not have constant
complexity**:

**24.3.12.2 Constructors   [inplace.vector.cons]**
...
> *Effects*: Constructs an `inplace_vector` object with the elements of the
> range `rg` [using the specified allocator]{.add}.

> *Complexity*: Linear in `ranges::distance(rg)`.

:::add
| `constexpr inplace_vector(inplace_vector&& v)`
|      `noexcept(N == 0 || @*is-nothrow-ua-constructible-v*@<T, Allocator, T&&>);`

> *Effects*: Equivalent to

>|     `inplace_vector(make_move_iterator(v.begin()),`
>|                                 `make_move_iterator(v.end()), v.get_allocator());`
>|     `v.clear();`.

| `constexpr inplace_vector(inplace_vector&& v, const Allocator& a)`
|     `noexcept(N == 0 || @*is-nothrow-ua-constructible-v*@<T, Allocator, T&&>);`

> *Effects*: Equivalent to

>|     `inplace_vector(make_move_iterator(v.begin()),`
>|                                 `make_move_iterator(v.end()), a);`
>|     `v.clear();`.

:::

### Changes to `inplace_vector::swap`

Add the definition of `swap` to the end of [inplace.vector.modifiers]{.sref}.
**Note that this definition of swap, in addition to addressing the allocator,
also fixes a pre-existing issue with `inplace_vector::swap` -- that, unlike for
other containers, it does not have constant complexity**:
...
**Design Note**: The behavior of `swap` described above works for
the *uses-allocator* design, but not for the `construct/destroy` design. In the
latter case, if the allocator propagates on swap, we need an additional mandate
that it is always possible to construct (not allocate) an object using one
allocator and destroy (not deallocate) it using a different allocator of the
same type, as described in the section,
[Behavior of move assignment and `swap`](#behavior-of-move-assignment-and-swap).
The *Effects* clause will be largely the same, but a complex *Mandates* clause
would likely be needed.


### Changes to Erasure ([inplace.vector.erasure]{.sref})
...
Thanks to Arthur O'Dwyer for helping me identify problems and solutions and for
