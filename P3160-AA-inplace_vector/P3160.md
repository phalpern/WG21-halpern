...
control over memory allocation. It would seem ironic is such a type did not fit
...
The proposal is to add make `inplace_vector` an *allocator-aware container* as
described in [container.alloc.reqmts]{.sref}.  Specifically:
...
([allocator.uses.construction]{.sref}), whether or not the allocator is a
scoped allocator.  The reasoning is that, since the `tuple` does not itself
allocate memory, it makes no sense to pass in an allocator that is compatible
with `T` but which is not passed to the wrapped `T` object.  The same logic
...
constructed using `allocator_traits<Allocator>::construct`. A non-intuitive
...
elements; other allocator types would effectively be ignored (except that they
...
* *Uses-allocator construction* is consistent with other non-allocating types
...
sub-option, although sub-option (a) could be used as an implementation
optimization.
...
**Sub-options for reducing wasted footprint storage** for non-AA elements:
...
c) Supplying an allocator other than `std::allocator` to for a type that cannot
...
using allocator-aware element types.
...
**Pros**: Easy to avoid accidentally forgetting to specify an allocator
parameter when using allocator-aware types (like Option 2), but automatic
...
The [run time](#run-time-results) and
...
5 identical files, each of which `#include`d the test program. Compilation was
...
preformed on each option to test that it produced correct results, run times
...
option 1 produced no penalty when the element types *does* use an allocator,
provided that allocator was not specifically provided to `inplace_vector`,
...
for using the default allocator parameter; any penalty would be born
