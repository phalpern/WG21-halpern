...
* `std::vector` is preferred over other containers in many cases because its
  ...
  structure defaults to `vector` for this reason. <!-- lah: Does "for this
  reason" refer to "...LEWG has spent a lot of time, for this reason, on ..."
  or does it refer to "...defaults, for this reason, to `vector`"? -->
...
memory-related performance optimizations. <!-- lah: what final format are you
using? The VS Code Markdown preview window doesn't properly convert --- to an
em dash, so we'll need to be sure your final format does. -->
...
4. **The Standard Library should encapsulate complexity**. Fully general <!--
   lah: "fully general" is a little unclear to me. --> application of
...
The proposal offered here is to add `inplace_vector` <!-- lah: "to add TO
`inplace_vector` an allocator-aware container"? or "to add `inplace_vector TO
an allocator-aware container"? --> an *allocator-aware container* as described
...
<!-- lah: 1. Check the code in the final format. In my Markdown preview window,
the last "deleted" gets wrapped to the next line. 2. freestanding-deleted what?
The hyphen implies this is an adjective, but the noun it describes is missing.
-->
...
* For most scoped allocators including `pmr::polymorphic_allocator`, <!-- lah:
  Do you mean "For most scoped allocators that include
  `pmr::polymorphic_allocator`, the two..." or do you mean ""For most scoped
  allocators, including `pmr::polymorphic_allocator`, the two..."? --> the two
...
allocator-aware inplace vector.<!-- lah: should this be `inplace_vector`? -->
...
**Cons**: The two types of inplace vector <!-- lah: should this be
`inplace_vector`? --> are not compatible.  The user needs to make a decision,
