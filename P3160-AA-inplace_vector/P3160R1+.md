...
document: P3160R1
date: <!-- $TimeStamp$ -->2024-10-09 15:26 EDT<!-- $ -->
audience: LEWG
author:
  - name: Pablo Halpern
...
* For most scoped allocators, including `pmr::polymorphic_allocator`, the two
  ...
  top level mechanism to construct objects, but not to allocate memory; the
  second-level allocator would be passed to the elements. The *uses-allocator*
  design would pass the top-level allocator to the constructed elements.  It is
  not clear that either design is more intuitive than the other.

* Allocator adaptors could let each design behave like the other.
...
considered.  Most are .  Option 1 --- with none of its suboptions --- is being proposed
here.
